...
/**
 * 기존에 @Configuration + @Bean + ProxyFactory를 이용한 프록시 방식 예시.
 * 현재는 @Aspect 기반 MetricsAspect로 대체되었으므로 비활성화 상태입니다.
 *
 * UserService를 직접 @Service로 등록하지 않고,
 * 이 설정 클래스에서 CGLIB 기반 프록시 객체로 감싸 스프링 컨테이너에 등록한다.
 *
 * 프록시는 메소드 호출 시 시작 시각, 종료 시각, 총 소요 시간을 로그로 출력한다.
 */
@Slf4j
// @Configuration  ← MetricsAspect로 대체됨
